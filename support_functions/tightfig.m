...
%         set(hax(i), 'LooseInset', [0,0,0,0]);
    ...
    moveleft = 0.8*min(pos(:,1) - ti(:,1));
    
    movedown = 0.8*min(pos(:,2) - ti(:,2));
    
    % we will also alter the height and width of the figure to just
    % encompass the topmost and rightmost axes and lables
    figwidth = 1.05*max(pos(:,1) + pos(:,3) + ti(:,3) - moveleft);
    
    figheight = 1.05*max(pos(:,2) + pos(:,4) + ti(:,4) - movedown);
